...
class Producto(db.Model):  # Producto hereda de db.Model
    ...
    def __init__(self, nombre, precio, stock, imagen, autor, editorial, sinopsis):
        """
...
class ProductoSchema(ma.Schema):
    ...
    """
    class Meta:
        fields = ("id", "nombre", "precio", "stock", "imagen", "autor", "editorial", "sinopsis")
...
def get_books(texto):
    ...
    """
    text_low = texto.lower()  # Convertir la cadena de búsqueda a minúscula
    all_productos = Producto.query.filter(Producto.nombre.ilike(f'%{text_low}%') | Producto.autor.ilike(f'%{text_low}%') | Producto.editorial.ilike(f'%{text_low}%')).all()  # Filtra los productos
    result = productos_schema.dump(all_productos)  # Serializa los registros en formato JSON
    return jsonify(result)  # Retorna el JSON de todos los registros de la tabla
...
def create_producto():
    ...
    new_producto = Producto(nombre, precio, stock, imagen, autor, editorial, sinopsis)  # Crea un nuevo objeto Producto con los datos proporcionados
...
def update_producto(id):
    ...
    producto.sinopsis = request.json["sinopsis"]
